...
def train(model, optimizer, train_loader):
    ...
    for data in train_loader:
        ...
        # print("train : ", y.shape)
        loss = F.mse_loss(out1.float(), y.float())
        optimizer.zero_grad()
        loss.backward()
        ...
        R2 = r2_score(train_label, train_pre)
    return R2,sqrt(total_loss / total_examples), train_pre, train_label
...
def test(loader, model):
    ...
    for data in loader:
        ...
        total_examples += data.num_graphs
        total_loss += float(loss) * data.num_graphs
        R2 = r2_score(label1, pre1)

    return R2, sqrt(total_loss / total_examples), pre1, label1  # ,total_labels.numpy().flatten(),total_preds.numpy().flatten()
...
def get_task_names(path, use_compound_names=False):
    ...
    return task_names
task_names = get_task_names('data/precessed/raw/freesolv.csv')
dataset = 'freesolv'
...
for train_index, val_index in kf.split(train_data):
    ...
    test_loader = DataLoader(val_fold, batch_size=64, shuffle=True)
    ...
    for epoch in range(epochs):
        ...
        train_loss, rmse1, pre2, label2 = train(model1, optimizer, train_loader)
        test_loss, rmse2, pre, label = test(test_loader, model1)
        #         score = metrics.r2_score(true, prediction)
        #         , true, prediction
        print(f'Epoch: {epoch:03d}, R2: {train_loss:.4f} '
              f'Test: {rmse1:.4f} ')  # f'score: {score:.4f} '
        print(f'Epoch: {epoch:03d}, R2: {test_loss:.4f} '
              f'Test: {rmse2:.4f} ')  # f'score: {score:.4f} '

        ret = [epoch, train_loss, test_loss]
        ...
        if epoch == 300:
            ...
            output1.to_csv("output/lipo_loss_curve.csv")
            # print(output)
