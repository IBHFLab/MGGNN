...
#         train_predictions += train_predictions + out1
    ...
    return total_loss/total_examples,train_pre,train_label
...
task_names = get_task_names('data/precessed/raw/lipo.csv')
...
for train_index, val_index in kf.split(train_data):
    ...
    df = pd.read_csv('data/precessed/raw/lipo.csv')
    # 假设要保存第 1、3、5 行的数据
    selected_data = df.iloc[train_index]  # 使用 iloc 方法选择指定行
    selected_data.to_csv('训练集测试集/lipo_train.csv', index=True)
    selected_data = df.iloc[val_index]  # 使用 iloc 方法选择指定行
    selected_data.to_csv('训练集测试集/lipo_test.csv', index=True)
...
for train_index, val_index in kf.split(train_data):
    ...
    df=pd.read_csv('esol.csv')
      # 假设要保存第 1、3、5 行的数据
    selected_data = df.iloc[train_index]  # 使用 iloc 方法选择指定行
    selected_data.to_csv('esol_train.csv', index=True)
    selected_data = df.iloc[val_index]  # 使用 iloc 方法选择指定行
    selected_data.to_csv('esol_test.csv', index=True)
    ...
    for epoch in range(epochs):
        ...
        print(f'Epoch: {epoch:03d}, Loss: {train_loss:.4f} '
              f'Test: {test_loss:.4f} ')  # f'score: {score:.4f} '
        ...
        if epoch==60:
            ...
            for data in test_loader:
                ...
                plt.show()
        if epoch == 100:
            pre_label = {'label': label2, 'pre': pre}
            ...
            output1.to_csv(str(i) + "bbbp_loss_curve.csv")
        #
