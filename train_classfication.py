...
class GraphAttentionLayer(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super(GraphAttentionLayer, self).__init__(aggr='add')  # 使用加法聚合
        self.in_channels = in_channels
        self.out_channels = out_channels

        self.query_layer = Linear(in_channels, out_channels)
        self.key_layer = Linear(in_channels, out_channels)
        self.value_layer = Linear(in_channels, out_channels)

        self.proj_layer = Linear(in_channels, out_channels)
    ...
    def forward(self, x, edge_index):
        ...
        # 开始传播
        return self.propagate(edge_index, size=(x.size(0), x.size(0)), x=x, query=query, key=key, value=value)

    def message(self, edge_index_i, query_i, key_j, value_j, size_i):
        # 计算注意力系数
        attention_scores = (query_i * key_j).sum(dim=-1)
        attention_weights = softmax(attention_scores, edge_index_i, num_nodes=size_i)

        # 应用注意力权重
        return attention_weights.view(-1, 1) * value_j
...
#原子表征layer
        ...
        self.attention=FeatureAttention(channels=hidden_channels, reduction=4)
        self.attentionlayer=GraphAttentionLayer(128,1)
    ...
    def forward(self, data) :
        ...
        for conv, gru in zip(self.atom_convs, self.atom_grus):
            h ,att1= conv(x, edge_index, return_attention_weights=True)
            h = F.elu(h)
            h = F.dropout(h, p=self.dropout, training=self.training)
            x = gru(h, x).relu()
...
def train(model, optimizer,train_loader):
    ...
    for data in train_loader:
        ...
        optimizer.zero_grad()
        loss.backward()
    #    auc_score = roc_auc_score(train_label, train_pre)
        optimizer.step()
...
#         train_predictions += train_predictions + out1
    ...
    return total_loss/total_examples,c,train_pre,train_label
...
def test(loader, model):
    ...
    for data in loader:
        ...
        total_loss += float(loss) * data.num_graphs
        total_examples += data.num_graphs
    return total_loss/total_examples,acc ,pre1,label1 #,total_labels.numpy().flatten(),total_preds.numpy().flatten()
#kfold交叉验证
train_task_names='bbbp'
dataset = 'bbbp'
...
for train_index, val_index in kf.split(train_data):
    ...
    test_loader = DataLoader(val_fold, batch_size=1, shuffle=True)
    model1 = New_model(in_channels=46,
                       hidden_channels=128,
                       out_channels=1,
                       edge_dim=10,
                       num_layers=3,
    ...
    for epoch in range(epochs):
        ...
        train_loss, auc,pre2,label2= train(model1, optimizer, train_loader)
        test_loss, test_acc, pre, label = test(test_loader, model1)
        #         score = metrics.r2_score(true, prediction)
        #         , true, prediction

        print(f'Epoch: {epoch:03d}, Loss: {train_loss:.4f} '
              f': {auc:.4f} ')  # f'score: {score:.4f} '

   #     ret = [epoch, train_acc, test_acc]

        train_losses.append(train_loss)
        val_losses.append(test_loss)
        #         scores.append(score)
        # Early Stopping
        the_current_loss = test_acc  # .item()
      ##  best_ret.append(ret)
        '''
        if the_current_loss < the_last_loss:
            ...
            ret = [epoch, train_acc, test_acc]  # , score
            trigger_times = 0
        ...
        if epoch ==22:
            pre_label = {'label': label2, 'pre': pre2}
            output = pd.DataFrame(pre_label)
            ...
            output1.to_csv("output/"+str(dataset)+"_loss_curve.csv")
            # print(output)
            output.to_csv("output/"+str(auc)+str(dataset)+".csv")
          #  torch.save(model1, 'bace.pt')
    ...
    output1 = pd.DataFrame(frame)
    # print(output)
    #output.to_csv("losses_test5.csv")


'''
#测试集训练集
train_task_names='bbbp_train'
dataset = 'bbbp'
task_type = 'classification'
tasks = train_task_names
train_data = MolDataset( root='data/precessed',dataset=dataset, task_type=task_type, tasks=tasks)
...
for epoch in range(epochs):
    optimizer = torch.optim.Adam(model1.parameters(), lr=learning_rate,
                                 weight_decay=weight_decay)
    train_loss, train_acc, pre2, label2 = train(model1, optimizer, train_loader)
    test_loss, test_acc, pre, label = test(test_loader, model1)
    #         score = metrics.r2_score(true, prediction)
    #         , true, prediction

    print(f'Epoch: {epoch:03d}, Loss: {train_acc:.4f} '
